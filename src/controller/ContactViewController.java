...
import static model.AppointmentDAO.getAppointmentsByContactId;
import static model.ContactDAO.getContactIdFromName;
import static model.ContactDAO.getContactList;


/**
 * Controller that shows appointments for each contact
 *
 */
public class ContactViewController implements Initializable {
    ...
    /**
     * Sets the table to show appointments based on the selected contact
     *
    ...
    void onActionComboBox(ActionEvent event) throws SQLException, ParseException {
       int id = getContactIdFromName(contactComboBox.getValue());
        aptTable.setItems(getAppointmentsByContactId(id));
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        try {
            contactComboBox.setItems(getContactList());
